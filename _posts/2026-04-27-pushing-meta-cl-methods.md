...
Our results also suggest that OML might be suffering from CF. Applying Kwon et al.'s <d-cite key="kwon_lifelearner_2024"></d-cite> approach to OML could help mitigate the CF seen in OML. We also struggled to get REMIND to perform well. We hypothesised that perhaps making more layers plastic could improve performance. Having seen that meta-CL can scale to 1,000 classes, future work should focus on how computationally efficient they are and whether they can generalise to datasets with different modalities.